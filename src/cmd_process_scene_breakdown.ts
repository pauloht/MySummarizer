...
async function runCharacterUpdateLLM(context: STContext, chatContent: string | undefined, existingCharacters: string[], summaryIndex: number): Promise<void> {
    let toast: object | null = null;
    try {
        ...
        if (characterBlocks.length === 0) return;
        ...
        const updates: Record<string, string | null> = JSON.parse(result);
